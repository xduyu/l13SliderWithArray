...
function renderExpense() {
    ...
    filteredExpense.forEach((el, index) => {
        ...
        liItem.innerHTML = `Название: ${el.name} <b> Сумма: ( ${el.sum} RUB ) </b>  Категория: ${el.category}`
...
function updateSum() {
    const sum = expense.reduce((countSum, el) => {
        return countSum + el.sum;
    }, 0);
    expensesTotal.textContent = `Общая сумма: ${sum}`;
}